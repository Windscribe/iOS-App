...
extension VPNManagerUtils {
    func configureOpenVPNWithSavedCredentials(with selectedNode: SelectedNode?,
        ...
        logger.logD(VPNManagerUtils.self, "Configuring VPN profile with saved credentials. \(String(describing: serverAddress))")
        ...
        guard base64username != "" && base64password != "" else {
            logger.logE(VPNManagerUtils.self, "Can't establish a VPN connection, missing authentication values.")
    ...
    func configureOpenVPNWithCustomConfig(with selectedNode: SelectedNode?,
        ...
        self.logger.logD(VPNManagerUtils.self, "Configuring VPN profile with custom configuration. \(String(describing: selectedNode.serverAddress))")
    ...
    func configureOpenVPN(manager: NEVPNManager,
        ...
        logger.logD(VPNManagerUtils.self, "VPN configuration successful. Username: \(username)")
    ...
    func getConfiguration(selectedNode: SelectedNode?,
        ...
        if let customConfig = selectedNode?.customConfig,
           ...
           let authRequired = customConfig.authRequired {
            ...
            self.logger.logD(VPNManagerUtils.self, proxyLine?.debugDescription ?? "")
