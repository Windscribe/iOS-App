...
class ServerSectionCell: UITableViewCell {
    ...
    func updateUI() {
        ...
        if let serverName = displayingServer?.name, let countryCode = displayingServer?.countryCode, let premiumOnly = displayingServer?.premiumOnly, let isUserPro = sessionManager.session?.isPremium, let isP2p = displayingServer?.p2p {
            ...
            if premiumOnly && !isUserPro {
                proIcon.image = UIImage(named: ImagesAsset.proServerIcon)
            }
            if isP2p == true {
                p2pImage.isHidden = true
            } else {
                p2pImage.isHidden = false
            }
        }
    }
    ...
    func expand(completion: @escaping () -> Void = {}) {
        if cellDivider.layer.opacity != 1.0 {
    ...
    func showForExpand() {
        cellDividerFull.frame.size.width = frame.width
        cellDivider.layer.opacity = 1.0
        serverNameLabel.layer.opacity = 1.0
        iconView.layer.opacity = 1.0
        flagIcon.layer.shadowOpacity = 1.0
        iconView.transform = CGAffineTransform(rotationAngle: .pi / 4)
        serverHealthView.isHidden = true
    }

    func collapse(completion: @escaping () -> Void = {}) {
        if cellDivider.layer.opacity != 0.05 {
            UIView.animate(withDuration: 0.35, animations: {
    ...
    func showForCollapse() {
        cellDividerFull.frame.size.width = 0
        cellDivider.layer.opacity = 0.05
        serverNameLabel.layer.opacity = 0.4
        iconView.layer.opacity = 1
        flagIcon.layer.shadowOpacity = 0.1
        iconView.transform = CGAffineTransform(rotationAngle: .pi * 4)
        serverHealthView.isHidden = false
    }
