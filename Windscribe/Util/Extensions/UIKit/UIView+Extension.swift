...
extension UIView {
    ...
    private static let lookAndFeelRepository = Assembler.resolve(LookAndFeelRepositoryType.self)
...
extension UIView {
    static func divider(color: UIColor? = nil, height: CGFloat = 1) -> UIView {
        let view = UIView()
        view.backgroundColor = color ?? (lookAndFeelRepository.isDarkMode ? UIColor.whiteWithOpacity(opacity: 0.08) : UIColor.midnightWithOpacity(opacity: 0.08))
