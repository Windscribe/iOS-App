...
class SecuredNetworkRepositoryImpl: SecuredNetworkRepository {
    ...
    private func loadNetworks() {
        ...
        localdatabase.getNetworks().subscribe(
            onNext: { savedNetworks in
                self.logger.logD("SecuredNetworkRepository", "Secured Networks : \(savedNetworks)")
                self.networks.onNext(savedNetworks)
            }, onError: { _ in
                self.logger.logD("SecuredNetworkRepository", "Error getting networks")
                self.networks.onNext([])
            }, onCompleted: {
                self.logger.logD("SecuredNetworkRepository", "getNetworks onCompleted")
    ...
    func getCurrentNetwork() -> WifiNetwork? {
        self.logger.logD("SecuredNetworkRepository", "connectivity.getNetwork() \(connectivity.getNetwork())")
